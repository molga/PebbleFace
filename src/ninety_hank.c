...
//  time_text2[0] = time_text[1];
 // text_layer_set_text(second_layer1, time_text1);
  ...
  if (the_last_minute != display_minute){  //check only every minute
	  ...
	  short minutes_tz1_correction = (short)((AdditionalTimezone_1 - floor(AdditionalTimezone_1)) * 60);
    short display_minute_tz1 = display_minute + minutes_tz1_correction;
    short hour_tz1_correction;
    if(display_minute_tz1 > 60)
    {
       display_minute_tz1 -= 60;
       hour_tz1_correction = 1;
    }
    if(display_minute_tz1 < 0)
    {
       display_minute_tz1 += 60;
       hour_tz1_correction = -1;
    }
    
    short  display_hour_tz1 = display_hour + (int)floor(AdditionalTimezone_1) + hour_tz1_correction;
	  ...
	  set_container_image(&date_digits_images[6], date_digits_layers[6], DATENUM_IMAGE_RESOURCE_IDS[display_minute_tz1/10], GPoint(108, 5));
	  set_container_image(&date_digits_images[7], date_digits_layers[7], DATENUM_IMAGE_RESOURCE_IDS[display_minute_tz1%10], GPoint(121, 5));  
	  // ======== Time Zone 1   
	  
	  // ======== Time Zone 2  
	  text_layer_set_text(text_addTimeZone2_layer, AdditionalTimezone_2_Description); 
    
    short minutes_tz2_correction = (short)((AdditionalTimezone_2 - floor(AdditionalTimezone_2)) * 60);
    short display_minute_tz2 = display_minute + minutes_tz2_correction;
    short hour_tz2_correction;
    if(display_minute_tz2 > 60)
    {
       display_minute_tz2 -= 60;
       hour_tz2_correction = 1;
    }
    if(display_minute_tz2 < 0)
    {
       display_minute_tz2 += 60;
       hour_tz2_correction = -1;
    }
    
	  short  display_hour_tz2 = display_hour + (int)floor(AdditionalTimezone_2) + hour_tz2_correction;
	  ...
	  set_container_image(&date_digits_images[10], date_digits_layers[10], DATENUM_IMAGE_RESOURCE_IDS[display_minute_tz2/10], GPoint(108, 26));
	  set_container_image(&date_digits_images[11], date_digits_layers[11], DATENUM_IMAGE_RESOURCE_IDS[display_minute_tz2%10], GPoint(121, 26));  
	  // ======== Time Zone 2 	  
...
//  layer_add_child(window_layer, text_layer_get_layer(second_layer2));  
 ...
 // update_display(tick_time); // causes wrong time displayed initially
  ...
  // Second tick // Modified on Minutes ticks because no need second display
